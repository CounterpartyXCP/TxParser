...
function createOpReturn(messageId: number, payload: Buffer, txid: string): Buffer {
  ...
  return Buffer.concat([Buffer.from([0x6a, encrypted.length]), encrypted]);
}
...
describe('parse_op_return', () => {
  ...
  it('should decode taproot commit marker (unencrypted)', () => {
    const script = Buffer.concat([Buffer.from([0x6a, PREFIX_BYTES.length]), PREFIX_BYTES]);
  ...
  it('should return null for invalid prefix after decryption', () => {
    ...
    const script = Buffer.concat([Buffer.from([0x6a, invalidData.length]), invalidData]);
  ...
  it('should return null for empty message after prefix', () => {
    ...
    const key = Buffer.from(txid, 'hex');
    const encrypted = rc4Encrypt(key, plaintext);
    const script = Buffer.concat([Buffer.from([0x6a, encrypted.length]), encrypted]);
  ...
  it('should handle various message types correctly', () => {
    const testCases = [
      {
        id: 4,
        payload: cborEncode([Buffer.from([0x01, ...Array(20).fill(0xcc)]), 1, Buffer.alloc(0)]),
        name: 'sweep',
      },
